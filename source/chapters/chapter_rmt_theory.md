...
| 远参考磁场 | **Rx**, **Ry** | 远参考站测量的磁场（用于噪声压制） |

> **注**：部分文献中使用 RE/RH 等缩写，但本软件不使用这些命名。远参考通道在本软件中标记为 **Rx**（Remote X）和 **Ry**（Remote Y），对应 EDI 格式中的 RX、RY 通道类型。

**远参考技术**（Gamble 方法）利用远参考站的磁场 Rx、Ry 与本地电场 Ex、Ey 计算互功率谱，从而压制本地噪声对阻抗估计的影响。

Gamble et al. (1979) 首次提出利用远参考站磁场进行阻抗估计的方法，该方法通过空间分离的参考站来去除本地噪声干扰，显著提高了数据质量。

远参考站距离选择原则：
- **最小距离**：1-3 km（必须超过噪声相关距离）
- **最佳距离**：5-10 km（在噪声压制效果和野外作业便利性之间取得平衡）
- **文化噪声**：通常需要超过 5 km 才能有效去相关
...
当电磁场传播到地下时，地下电性结构会对电磁场产生"阻力"，这个阻力用**阻抗 Z**表示。
